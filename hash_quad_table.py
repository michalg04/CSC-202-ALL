...
class HashTableQuadPr:
    ...
    # HashTableQuadPr file hashtable =>
        ...
        try:
            ...
            with open(filename, 'r', encoding='utf-8') as file:
                ...
                for line in file:
                    ...
                    words = line.split()
                    for word in words:
                        #inserts all the words that aren't numbers or stop words
                        if not self.is_number(word) and not stop_table.find(word):
                            self.insert(word, counter)
        # if file doesn't exist
        except IOError:
            return 'file does not exists'

    # HashTableQuadPr int => boolean
    ...
    def find(self, key):
        ...
        for i in range(self.table_size):
            ...
            (key1, items1) = self.hash_table[(hash_value + i*i)% self.table_size]
            if key1 == '':
                return False
            if key1 == key:
                return True
    ...
    def insert_helper(self, key, items):
        ...
        #print(hash_value)
        for i in range(self.table_size):
            index = (hash_value + i*i)% self.table_size
